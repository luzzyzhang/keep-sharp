...
def singleton(instance):
    ...
    if hasattr(klass, '__instantiated'):
        msg = "%s is a Singleton class but is already instantiated"
        raise ValueError(msg % klass)
...
class Singleton(object):
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, '_instances'):
            # cls._instances = super(Singleton, cls). __new__(*args, **kwargs)
...
# 这个装饰器实现感觉更好一些:)
def singleton(cls):
    def _sinlge(*args, **kwargs):
        if not hasattr(cls, '_instanced'):
            cls._instanced = cls(*args, **kwargs)
